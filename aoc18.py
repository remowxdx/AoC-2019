...
class Node:
    ...
    def shortcut(self):
        if len(self.links) == 1 and self.type == '.':
            for l in self.links:
                pass
            l.remove_link(self)
            return
...
class Maze:
    ...
    def bfs(self):
        ...
        to_visit = [(0, tuple(self.starts), ())]
        mkl = 0
        while len(to_visit) > 0:
            l, nodes, keys = heapq.heappop(to_visit)
            
            if len(keys) == self.num_keys:
                return l
            if len(keys) > mkl:
                pd(len(keys), len(to_visit))
                mkl = len(keys)
                ...
                for new_node in n.links:
                    ...
                    for j, nn in enumerate(nodes):
                        if i == j:
                            ...
                            new_nodes.append(nn)
                    heapq.heappush(to_visit, (l + d, tuple(new_nodes), new_keys))
...
'''
    if True:
        ...
        print('Test Part 2:')
        test_eq('Test 2.1', part2, 32, test_input_6)
        test_eq('Test 2.2', part2, 72, test_input_7)
        print()
